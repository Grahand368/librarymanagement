...
<head>
    ...
    <title>Manage Categories · Aurora Library</title>
    ...
    <main class="main">
        <div class="page-head fade-up">
            <div>
                ...
                <h1 class="page-head__title">Manage <em>Categories</em></h1>
                <p class="page-head__sub">Edit, update, or remove book categories.</p>
            </div>
                ...
                <?php
                    ...
                    if(mysqli_num_rows($query_run) > 0){
                            ...
                            echo '<tr>
                                <td>
                                    <div class="flex gap-3" style="align-items:center;">
                                        <div style="width:36px;height:36px;border-radius:10px;background:linear-gradient(135deg,var(--gold-100),var(--gold-200));color:var(--gold-900);display:grid;place-items:center;font-weight:700;">'.strtoupper(substr($row['cat_name'], 0, 1)).'</div>
                                        <div class="table__title">'.htmlspecialchars($row['cat_name']).'</div>
                                    </div>
                                </td>
                                <td style="text-align:right;">
                                    <div class="flex gap-2" style="justify-content:flex-end;">
                                        <a href="edit_cat.php?cid='.urlencode($row['cat_id']).'" class="btn btn--sm btn--ghost" style="padding:6px 12px;">
                                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>
                                            Edit
                                        </a>
                                        <a href="delete_cat.php?cid='.urlencode($row['cat_id']).'" data-confirm="Delete this category permanently?" class="btn btn--sm" style="padding:6px 12px;background:rgba(155,28,28,0.10);color:var(--crimson);">
                        ...
                        echo '<tr><td colspan="2" style="text-align:center;padding:60px 20px;">
                            ...
                            <p style="color:var(--ink-500);">No categories yet. Create your first one.</p>
                        </td></tr>';
                    }
                ?>
                </tbody>
